...
def main(local_rank, args):
    ...
    while epoch < max_num_epochs:
        ...
        for i_iter, data in enumerate(train_dataset_loader):
            (points, train_vox_label, train_grid, train_pt_labs, train_grid_vox) = data
            train_grid_vox = train_grid_vox.to(torch.float32).cuda()
            points = points.cuda()
            ...
            with torch.cuda.amp.autocast():
                outputs_vox, outputs_pts = my_model(points=points, grid_ind=train_grid, grid_ind_vox=train_grid_vox)
